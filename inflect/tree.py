...
class Word(object):
    ...
    def copy(self, chunk=None, pnp=None):
        w = Word(self.sentence, self.string, self.lemma, self.type, self.index)
        w.chunk = chunk
        w.pnp = pnp
    ...
    def tags(self):
        ...
        for i, tag in enumerate(
            ...
            if tag == WORD:
                ...
                tags[i] = [
                    "-".join([str(x) for x in [ch.type] + list(reversed(r)) if x])
...
class Chunk(object):
    ...
    def related(self):
        ...
        return [
            ch
            for ch in self.sentence.chunks
            if ch != self
            and intersects(unzip(0, ch.relations), unzip(0, self.relations))
        ]
    ...
    def anchor_id(self):
        ...
        id = ""
        f = lambda ch: list(
            filter(lambda k: self.sentence._anchors[k] == ch, self.sentence._anchors)
        )
...
class Sentence(object):
    ...
    def __init__(
        ...
        self._previous = (
            None  # Helper variable: the last token parsed in parse_token().
        )
    ...
    def parse_token(self, token, tags=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA]):
        ...
        for k, v in zip(tags, token.split("/")):
            ...
            if k not in p:
                custom[k] = None
            if (
                v != OUTSIDE or k == WORD or k == LEMMA
    ...
    def _do_anchor(self, anchor):
        ...
        if anchor:
            for x in anchor.split("-"):
                ...
                if (
                    A in self._anchors
                    and P in self._anchors
                    and not self._anchors[P].anchor
                ):
                    pnp = self._anchors[P]
    ...
    def slice(self, start, stop):
        ...
        for i, word in enumerate(self.words[start:stop]):
            ...
            p5 = (
                word.chunk is not None and unzip(0, word.chunk.relations) or None
            )  # REL
            p6 = (
                word.chunk is not None and unzip(1, word.chunk.relations) or None
            )  # ROLE
    ...
    def __repr__(self):
        return "Sentence(%s)" % repr(
            " ".join(["/".join(word.tags) for word in self.words])
        )
...
def parse_xml(sentence, tab="\t", id=""):
    ...
    """
    uid = lambda *parts: "".join(
        [str(id), _UID_SEPARATOR] + [str(x) for x in parts]
    ).lstrip(_UID_SEPARATOR)
    ...
    for word in sentence.words:
        ...
        if pnp and pnp.start == chunk.start and pnp.start == word.index:
            a = (
                pnp.anchor
                and ' %s="%s"' % (XML_OF, uid("A", anchors.get(pnp.anchor.start, "")))
                or ""
            )
        ...
        if chunk and chunk.start == word.index:
            ...
            xml.append(
                ...
                % (
                    ...
                    chunk.relations
                    and chunk.role is not None
                    and ' %s="%s"' % (XML_RELATION, r2)
                    or "",
                    chunk.relation
                    and chunk.type == "VP"
                    and ' %s="%s"' % (XML_ID, uid(chunk.relation))
                    or "",
                    chunk.relation
                    and chunk.type != "VP"
                    and ' %s="%s"' % (XML_OF, r1)
                    or "",
                    chunk.attachments
                    and ' %s="%s"' % (XML_ANCHOR, uid("A", anchors[chunk.start]))
                    or "",
                )
            )
            indent = push(indent)
            ...
            % (
                ...
                (
                    ...
                    + " ".join(
                        [
                            '%s="%s"' % (k, v)
                            for k, v in word.custom_tags.items()
                            if v is not None
                        ]
                    )
                ).rstrip(),
...
_anchors: dict[str, list[list[str]]] = (
    {}
)  # {'A1': [['eat', 'VBP', 'B-VP', 'O', 'VP-1', 'O', 'eat', 'O']]}
_attachments: dict[str, list[list[str]]] = (
    {}
)  # {'A1': [[['with', 'IN', 'B-PP', 'B-PNP', 'PP', 'O', 'with', 'O'],
...
def parse_string(xml):
    ...
    for sentence in dom(XML_SENTENCE):
        ...
        format = (
            not isinstance(format, str) and format or format.replace(" ", "").split(",")
        )
        ...
        if ANCHOR in format:
            ...
            for id in sorted(A.keys()):
                for token in A[id]:
                    token[i] += "-" + "-".join(
                        ["A" + str(a + p) for p in range(len(P[id]))]
                    )
...
def _parse_relation(chunk, type="O"):
    ...
    return ";".join(
        ["-".join([x for x in (type, r1, r2) if x]) for r1, r2 in zip(r1, r2)]
    )
...
def graphviz_dot(sentence, font="Arial", colors=BLUE):
    ...
    s += "\tnodesep=0.15;\n"
    s += (
        '\tnode [penwidth=1, fontname="%s", shape=record, margin=0.1, height=0.35];\n'
        % font
    )
    s += "\tedge [penwidth=1];\n"
    s += "\t{ rank=same;\n"
    ...
    for i, ch in enumerate(sentence.chunks):
        s += '\t\tchunk%s [label="<f0>%s"%s];\n' % (
            i + 1,
            "-".join([x for x in (ch.type, ch.role, str(ch.relation or "")) if x])
            or "-",
            _colorize(ch, colors),
        )
...
def table(sentence, fill=1, placeholder="-"):
    ...
    columns[3] = [
        columns[3][i] + (iob == "I" and " ^" or "") for i, iob in enumerate(columns[2])
    ]
    del columns[2]
    for i, header in enumerate(
        ["word", "tag", "chunk", "role", "id", "pnp", "anchor", "lemma"] + tags[9:]
    ):
        columns[i].insert(0, "")
